...
*/    
contract CryptoCoupons is AccessControl, DetailedERC721 {
    using SafeMath for uint256;

    event TokenCreated(uint256 tokenId, string name, string serialNumber, uint256 price, address owner);
    event TokenSold(
    ...
    function createToken(string _name, string _description, string _serialNumber, bool _gift, uint _fee, uint8 _value,
    ...
    function createToken(string _name, string _description, bool _gift, uint8 _value) public onlyCEO {
        ...
        cont++;
        _createToken(_name, _description, _serialNumber, _gift, 0, _value,
              cooAddress, 0.01 ether);
    }
    //Se debe evaluar el gas consumido en lotes 
     function createTokensBatch(string _name, string _description, bool _gift, uint8 _value, uint _numberOfTokens) public {
        for (uint i = 0; i < _numberOfTokens ; i++){
            _createToken(_name, _description, _appendUintToString(_name, i), _gift, 0, _value,
              cooAddress, 0.01 ether);
        }
        
    }
   
    function _createToken(string _name, string _description, string _serialNumber, bool _gift, uint _fee,
